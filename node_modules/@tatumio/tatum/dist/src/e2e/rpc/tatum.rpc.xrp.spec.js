...
const getXrpRpc = async (testnet) => await service_1.TatumSDK.init({
    network: testnet ? service_1.Network.XRP_TESTNET : service_1.Network.XRP,
    retryCount: 1,
    retryDelay: 2000,
});
describe.skip('RPCs', () => {
    ...
    describe('XRP', () => {
        describe('mainnet', () => {
            ...
            it('transaction_entry', async () => {
                const tatum = await getXrpRpc();
                const { result } = await tatum.rpc.transactionEntry('C53ECF838647FA5A4C780377025FEC7999AB4182590510CA461444B207AB74A9', { ledgerIndex: 56865245 });
                expect(result.tx_json.TransactionType).toBe('OfferCreate');
            });
            it('tx', async () => {
                const tatum = await getXrpRpc();
                const { result } = await tatum.rpc.tx('C53ECF838647FA5A4C780377025FEC7999AB4182590510CA461444B207AB74A9', {
                    binary: false,
                });
...
//# sourceMappingURL=tatum.rpc.xrp.spec.js.map