...
const getClient = async (testnet) => await service_1.TatumSDK.init({
    network: testnet ? service_1.Network.SOLANA_DEVNET : service_1.Network.SOLANA,
    retryCount: 1,
    retryDelay: 2000,
});
...
describe.skip('Solana mainnet RPC', () => {
    ...
    describe('getBlock', () => {
        it('should return a recent block', async () => {
            ...
            const { result } = await tatum.rpc.getBlock(slot || 0, {
                encoding: dto_1.Encoding.JsonParsed,
    ...
    describe('getBlocks', () => {
        ...
        it('should return an array of confirmed block numbers between two slots', async () => {
            ...
            const endSlot = 193167070;
            const { result } = await tatum.rpc.getBlocks(endSlot, startSlot, {
                commitment: dto_1.Commitment.Confirmed,
            });
            expect(Array.isArray(result)).toBe(true);
        });
    });
    ...
    describe('getTokenAccountsByOwner', () => {
        it('should return token accounts by owner', async () => {
            const tatum = await getClient();
            const { result } = await tatum.rpc.getTokenAccountsByOwner('GgPpTKg78vmzgDtP1DNn72CHAYjRdKY7AV6zgszoHCSa', {
...
//# sourceMappingURL=tatum.rpc.solana.spec.js.map